...
class patchTST_multiChannel(nn.Module):
    def __init__(self, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        if self.pretrain_head:
            ...
            self.head = self.create_pretrain_head(self.head_nf, self.n_vars_out, dropout)
        elif head_type == 'flatten':
            self.head = Flatten_Head(self.individual, 100, self.head_nf, target_window, dropout)

    def forward(self, input_z, time_stamp, full_data):
        batch_size = input_z.shape[0]
        # 添加掩码
        mask_z = (input_z != -9999).any(dim=(1, 2)).unsqueeze(-1).unsqueeze(-1)
        input_z = input_z * mask_z

        n_vars = input_z.shape[1]
        input_z = input_z.reshape(batch_size, n_vars, -1)
        total_len = input_z.shape[-1]
        input_z = input_z.reshape(-1, n_vars)

        input_z = self.norm(input_z).reshape(batch_size, -1, total_len)
        ...
        if self.memory:
            ...
            for i, influence in enumerate(self.memory):
                ...
                influence_channel = self.influence_embedding(
                    self.influence_norm[i](influence_channel).reshape(batch_size, -1, seq_len))
                influence_list.append(influence_channel)

            z_list = []
            for i in range(len(influence_list)):
                z_list.append(torch.concat([input_z[:, i:, ], influence_list[i]], dim=1))

            output_list = []
            for z in z_list:
                # 修改time_stamp
                if time_stamp is not None:
                    this_time_stamp = time_stamp[:, :, :self.context_window].clone()

                # do patching
                z = self.padding_patch_layer(z)
                this_time_stamp = self.padding_patch_layer(this_time_stamp.transpose(-2, -1))

                z = z.unfold(dimension=-1, size=self.patch_len,
                             step=self.stride)  # z: [bs x nvars x patch_num x patch_len]
                this_time_stamp = this_time_stamp.unfold(dimension=-1, size=self.patch_len, step=self.stride)

                z = z.permute(0, 1, 3, 2)  # z: [bs x nvars x patch_len x patch_num]
                this_time_stamp = this_time_stamp.permute(0, 1, 3, 2)

                # model
                z = self.backbone(z, this_time_stamp)  # z: [bs x nvars x d_model x seq_len]

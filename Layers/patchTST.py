...
class patchTST(nn.Module):
    def __init__(self, context_window: int, target_window: int, patch_len: int, stride: int,
                 ...
                 res_attention: bool = True, pre_norm: bool = False, pretrain_head: bool = False, head_type='flatten',
    ...
    def forward(self, z, time_stamp):
        # 添加掩码
        n_vars = len(self.target)
        batch_size = z.shape[0]
        mask = (z != -9999).any(dim=(1, 2)).unsqueeze(-1).unsqueeze(-1)
        z = z * mask
...
if __name__ == "__main__":
    patchTST = patchTST(context_window=1024, target_window=512, patch_len=32, stride=16, target=['NO2', 'PM25', 'O3'])

    x = torch.rand((32, 3, 1024))
    time_stamp = torch.rand((32, 1024, 1))

    y = patchTST(x, time_stamp)
